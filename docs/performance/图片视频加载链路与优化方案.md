...
`appendMergedVideoToSession` 已实现：合并成功后上传 OSS、写入会话 metadata，并登记到媒体中心（`source=STUDIO_MEDIA`）。
...
| B6 | `appendMergedVideoToSession` | **已解决**（2026-07-13 会话回写；2026-07-15 同步写入媒体中心） | 合并产物上传至 OSS、会话，并 `registerStudioMedia` |
...
| 2026-07-13 | **P1-5 实现**：`appendMergedVideoToSession` 上传合并 WebM 至 `POST /api/chats/.../media-results`，写入会话 assistant 消息并在 Studio 预览区展示 |
...
| 2026-07-15 | 合并/Studio 媒体保存时调用 `registerStudioMedia` 写入 `media_center_items`（`STUDIO_MEDIA` / `STUDIO_EDIT`），全局媒体中心可见。 |
